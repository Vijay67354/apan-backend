...
//     company: { type: String, required: true },
//     location: { type: String, required: true },
//     salary: { type: String, required: true }, // Changed to String to match job data format
//     experience: { type: String, required: true },
//     skills: [String],
//     postedAt: { type: Date, default: Date.now },
//     type: { type: String, enum: ['Work from home', 'Work from office', 'Work from field'], required: true },
...
// // TopCompanyJob Schema
// const topCompanyJobSchema = new mongoose.Schema({
//     title: { type: String, required: true },
//     company: { type: String, required: true },
//     location: { type: String, required: true },
//     salary: { type: String, required: true }, // Changed to String
//     experience: { type: String, required: true },
//     skills: [{ type: String }],
//     postedAt: { type: Date, default: Date.now },
...
// // Mock job data (with corrected duplicate ID: changed second id 23 to 24)
//   const jobs = [
//     {
//       id: 1,
//       designation: "Frontend Developer",
...
//                 type: 'Work from office',
//                 workType: 'Full-Time',
//                 level: 'Mid', // Added required field
//                 role: 'Frontend Developer', // Added required field
//                 description: 'Develop and maintain web applications.',
//                 responsibilities: 'Build UI components and integrate APIs.',
//                 openings: 3,
//                 requirements: ['Communication', 'Teamwork'],
...
//                 company: 'Google',
//                 location: 'Hyderabad, India',
//                 salary: '₹ 25-30 Lakhs P.A.',
//                 experience: '3 years',
//                 skills: ['Python', 'Machine Learning', 'SQL'],
//                 postedAt: new Date('2025-06-02'),
//                 type: 'Work from home',
//                 workType: 'Full-Time',
//                 level: 'Senior', // Added required field
//                 role: 'Data Scientist', // Added required field
//                 description: 'Analyze data and build predictive models.',
//                 responsibilities: 'Develop ML models and present insights.',
//                 openings: 2,
//                 requirements: ['Analytical Thinking', 'Problem Solving'],
...
//                 workType: 'Full-Time',
//                 level: 'Senior', // Added required field
//                 role: 'Product Manager', // Added required field
//                 description: 'Lead product development and strategy.',
//                 responsibilities: 'Define product roadmap and collaborate with teams.',
//                 openings: 1,
//                 requirements: ['Communication', 'Strategic Thinking'],
...
//                 openings: 2,
//                 requirements: ['Basic coding skills', 'Teamwork'],
//                 urgent: false,
//                 applicants: 10,
//                 img: 'https://via.placeholder.com/64',
//                 level: 'Junior', // Added required field
//                 role: 'Frontend Developer', // Added required field
//             },
//         ];
//         await TopCompanyJob.insertMany(topCompanyJobs);
...
//         }
//         const userId = req.body.userId || 'user1';
//         const resumes = JSON.parse(fs.readFileSync(path.join(__dirname, 'resumes.json'), 'utf8') || '{}');
//         resumes[userId] = req.file.path;
//         fs.writeFileSync(path.join(__dirname, 'resumes.json'), JSON.stringify(resumes, null, 2));
//         const fileDetails = {
//             filename: req.file.filename,
//             originalName: req.file.originalname,
//             size: req.file.size,
...
//     if (designation) {
//         filteredJobs = filteredJobs.filter((job) =>
//             job.designation.toLowerCase().includes(designation.toLowerCase().trim())
//         );
//     }
//     if (location) {
//         filteredJobs = filteredJobs.filter((job) =>
...
//         filteredJobs = filteredJobs.filter((job) => {
//             const jobExp = Number(job.experience.split(' ')[0]);
//             return maxExp ? jobExp >= minExp && jobExp <= maxExp : jobExp >= minExp;
//         });
//     }
//     console.log('Filtered Jobs in API:', filteredJobs);
//     res.json(filteredJobs);
...
//     console.log('Received GET request for /api/jobs/:jobId with jobId:', req.params.jobId);
//     try {
//         const job = await Job.findById(req.params.jobId) || jobs.find((job) => String(job.id) === req.params.jobId);
//         if (!job) {
//             return res.status(404).json({ success: false, message: 'Job not found' });
//         }
//         res.json({ success: true, data: job });
...
//         if (type) query.type = type;
//         if (level) query.level = level;
//         const dbJobs = await Job.find(query).sort({ postedAt: -1 });
//         res.json({ success: true, data: [...jobs, ...dbJobs] });
//     } catch (error) {
//         console.error('Error fetching jobs:', error);
//         res.status(500).json({ success: false, message: 'Server error' });
//     }
...
// Mock job data
const jobs = [
  // [Your jobs array here - truncated for brevity, include all 77 jobs]
  {
    id: 1,
    designation: "Frontend Developer",
    experience: "2 years",
...
// Seed Data (run only in development)
if (process.env.NODE_ENV !== 'production') {
  ...
  const insertTestUser = async () => {
    const existingUser = await Candidate.findOne({ email: process.env.ADMIN_EMAIL });
    if (!existingUser) {
      const hashedPassword = await bcrypt.hash(process.env.DEMO_PASSWORD || 'password123', 10);
      const newCandidate = new Candidate({
...
app.get('/api/jobs/search', (req, res) => {
  const { designation, experience, location } = req.query;
  let filteredJobs = jobs;
  if (designation) filteredJobs = filteredJobs.filter((job) => job.designation.toLowerCase().includes(designation.toLowerCase().trim()));
  if (location) filteredJobs = filteredJobs.filter((job) => job.location.toLowerCase().includes(location.toLowerCase().trim()));
  if (experience) {
    const [minExp, maxExp] = experience.split('-').map(Number);
    filteredJobs = filteredJobs.filter((job) => {
      const jobExp = Number(job.experience.split(' ')[0]);
...
app.get('/api/jobs/suggested', (req, res) => {
  const { location, designation } = req.query;
  let suggestedJobs = jobs;
  if (location) suggestedJobs = suggestedJobs.filter((job) => job.location.toLowerCase().includes(location.toLowerCase().trim()));
  if (designation) suggestedJobs = suggestedJobs.filter((job) => job.designation.toLowerCase().includes(designation.toLowerCase().trim()));
  res.status(200).json(suggestedJobs.slice(0, 5));
});
...
app.post('/api/jobs', async (req, res) => {
  try {
    ...
    const requiredFields = ['title', 'company', 'location', 'salary', 'experience', 'type', 'workType'];
    for (const field of requiredFields) {
      if (!jobData[field]) return res.status(400).json({ success: false, message: `${field} is required` });
    }
    const newJob = new Job({
      ...jobData,
      postedAt: jobData.postedAt ? new Date(jobData.postedAt) : Date.now(),
...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: 'Signup successful',
      user: { id: newUser._id, email: newUser.email, name: newUser.name, avatar: newUser.avatar },
    });
  } catch (error) {
    console.error('Signup error:', error);
    res.status(500).json({ error: 'Failed to sign up' });
...
app.post('/api/google-login', async (req, res) => {
  ...
  try {
    let user = await User.findOne({ googleId });
    if (!user) user = await User.create({ email, googleId, name, picture });
    res.json({ user });
  } catch (err) {
    console.error('Google login error:', err);
    res.status(500).json({ error: 'Google login failed' });
...
app.patch('/api/top-companies-jobs/:id/apply', async (req, res) => {
  try {
    const { id } = req.params;
    const { applicants } = req.body;
    if (typeof applicants !== 'number') return res.status(400).json({ message: 'Applicants count must be a number' });
    const job = await TopCompanyJob.findById(id);
